...
static inline void print_warning_list(const char *prefix, THD *thd)
{
...
static void ndbcluster_binlog_close_table(THD *thd, NDB_SHARE *share)
{
...
static int ndb_binlog_open_shadow_table(THD *thd, NDB_SHARE *share)
{
    int error;
...
*/
static THD *setup_thd(char *stackptr)
{
...
static int ndbcluster_binlog_index_purge_file(THD *thd, const char *file)
{
    int error = 0;
    THD *save_thd = thd;
...
static bool priv_tables_are_in_ndb(THD *thd)
{
...
static void ndbcluster_binlog_log_query(handlerton *hton, THD *thd, enum_binlog_command binlog_command,
...
*/
static size_t ndb_open_tables__create_key(char *key_buf, size_t key_buf_length,
...
*/
static bool ndb_open_tables__is_table_open(const char *db, size_t db_length,
        const char *table, size_t table_length)
{
...
static bool ndbcluster_check_ndb_schema_share()
{
...
static bool ndbcluster_check_ndb_apply_status_share()
{
    return ndb_open_tables__is_table_open(STRING_WITH_LEN("mysql"),
                                          STRING_WITH_LEN("ndb_apply_status"));
}
...
static bool create_cluster_sys_table(THD *thd, const char *db, size_t db_length,
                                     const char *table, size_t table_length,
...
static bool ndb_apply_table__create(THD *thd)
{
...
static bool ndb_schema_table__create(THD *thd)
{
...
*/
static int ndbcluster_update_slock(THD *thd,
                                   const char *db,
                                   const char *table_name,
                                   uint32 table_id,
                                   uint32 table_version)
...
int ndb_handle_schema_change(THD *thd, Ndb *is_ndb, NdbEventOperation *pOp,
...
static int ndb_binlog_thread_handle_schema_event(THD *thd, Ndb *s_ndb,
...
*/
static void ndb_binlog_thread_handle_schema_event_post_epoch(THD *thd,
        List<Cluster_schema>
        *post_epoch_log_list,
        List<Cluster_schema>
        *post_epoch_unlock_list)
...
*/
static int ndb_binlog_index_table__open(THD *thd,
...
*/
static int ndb_binlog_index_table__write_rows(THD *thd,
...
**************************************************************/
void ndb_rep_event_name(String *event_name, const char *db, const char *tbl,
                        my_bool full)
{
    if (full)
...
#ifdef HAVE_NDB_BINLOG
static void set_binlog_flags(NDB_SHARE *share,
...
static int slave_set_resolve_fn(THD *thd, NDB_SHARE *share,
...
*/
int row_conflict_fn_old(NDB_CONFLICT_FN_SHARE *cfn_share,
...
int row_conflict_fn_max_update_only(NDB_CONFLICT_FN_SHARE *cfn_share,
                                    enum_conflicting_op_type op_type,
                                    const uchar *old_data,
                                    const uchar *new_data,
                                    const MY_BITMAP *write_set,
...
*/
int row_conflict_fn_max(NDB_CONFLICT_FN_SHARE *cfn_share,
                        enum_conflicting_op_type op_type,
                        const uchar *old_data,
                        const uchar *new_data,
                        const MY_BITMAP *write_set,
...
int row_conflict_fn_max_del_win(NDB_CONFLICT_FN_SHARE *cfn_share,
                                enum_conflicting_op_type op_type,
                                const uchar *old_data,
                                const uchar *new_data,
                                const MY_BITMAP *write_set,
...
int row_conflict_fn_epoch(NDB_CONFLICT_FN_SHARE *cfn_share,
                          enum_conflicting_op_type op_type,
                          const uchar *old_data,
                          const uchar *new_data,
                          const MY_BITMAP *write_set,
...
static int setup_conflict_fn(THD *thd, NDB_SHARE *share,
                             const NDBTAB *ndbtab,
                             char *msg, uint msg_len,
                             TABLE *table,
                             const st_conflict_fn_def *conflict_fn,
...
/*
   ...
   returned.
*/
int ndbcluster_read_replication_table(THD *thd, Ndb *ndb,
                                      const char *db,
                                      const char *table_name,
                                      uint server_id,
                                      Uint32 *binlog_flags,
    ...
    *col_db, *col_table_name, *col_server_id, *col_binlog_type, *col_conflict_fn;
    ...
    for (;;) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            if ((_op = trans->getNdbOperation(reptab)) == NULL)
                abort();

            if (_op->readTuple(NdbOperation::LM_CommittedRead))
                abort();

            ndb_pack_varchar(col_db, tmp_buf, db, strlen(db));

            if (_op->equal(col_db->getColumnNo(), tmp_buf))
                abort();

            ndb_pack_varchar(col_table_name, tmp_buf, table_name, strlen(table_name));

            if (_op->equal(col_table_name->getColumnNo(), tmp_buf))
                abort();

            if (_op->equal(col_server_id->getColumnNo(), id))
                abort();

            if ((col_binlog_type_rec_attr[i] =
                        _op->getValue(col_binlog_type, (char *) & (ndb_binlog_type[i]))) == 0)
                abort();

            /* optional columns */
            if (col_conflict_fn) {
                if ((col_conflict_fn_rec_attr[i] =
                            _op->getValue(col_conflict_fn, ndb_conflict_fn[i])) == 0)
                    abort();
            }
...
*/
int ndbcluster_get_binlog_replication_info(THD *thd, Ndb *ndb,
        const char *db,
        const char *table_name,
        uint server_id,
        const TABLE *table,
...
int ndbcluster_apply_binlog_replication_info(THD *thd,
        NDB_SHARE *share,
        const NDBTAB *ndbtab,
        TABLE *table,
        const st_conflict_fn_def *conflict_fn,
...
int ndbcluster_read_binlog_replication(THD *thd, Ndb *ndb,
                                       NDB_SHARE *share,
                                       const NDBTAB *ndbtab,
                                       uint server_id,
                                       TABLE *table,
...
int ndbcluster_create_event(THD *thd, Ndb *ndb, const NDBTAB *ndbtab,
...
*/
int ndbcluster_create_event_ops(THD *thd, NDB_SHARE *share,
...
*/
static int ndb_binlog_thread_handle_error(Ndb *ndb,
        NdbEventOperation *pOp)
{
    Ndb_event_data *event_data = (Ndb_event_data *) pOp->getCustomData();
    NDB_SHARE *share = event_data->share;
...
static int ndb_binlog_thread_handle_non_data_event(THD *thd,
        NdbEventOperation *pOp,
        ndb_binlog_index_row &row)
{
    Ndb_event_data *event_data = (Ndb_event_data *) pOp->getCustomData();
...
*/
inline ndb_binlog_index_row *ndb_find_binlog_index_row(ndb_binlog_index_row **rows,
...
static int ndb_binlog_thread_handle_data_event(Ndb *ndb, NdbEventOperation *pOp,
