...
/**
  ...
  @todo Move this function into the cache class?
 */
static int gtid_before_write_cache(THD *thd, binlog_cache_data *cache_data)
{
...
template <class RowsEventT> Rows_log_event *THD::binlog_prepare_pending_rows_event(TABLE *table, uint32 serv_id,
        size_t needed,
        bool is_transactional,
...
*/
bool THD::binlog_row_event_extra_data_eq(const uchar *a,
        const uchar *b)
{
    return ((a == b) ||
            ((a != NULL) &&
