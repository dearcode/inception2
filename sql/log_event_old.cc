...
int Old_rows_log_event::do_apply_event(Old_rows_log_event *ev, const Relay_log_info *rli)
{
 ...
 */
static int copy_extra_record_fields(TABLE *table,
 ...
 */
static int replace_record(THD *thd, TABLE *table,
...
int Write_rows_log_event_old::do_prepare_row(THD *thd_arg,
...
int Delete_rows_log_event_old::do_prepare_row(THD *thd_arg,
        Relay_log_info const *rli,
        TABLE *table,
        uchar const *row_start,
        uchar const **row_end)
...
Log_event::enum_skip_reason Old_rows_log_event::do_shall_skip(Relay_log_info *rli)
{
    /*
...
int Old_rows_log_event::do_update_pos(Relay_log_info *rli)
{
...
int Old_rows_log_event::write_row(const Relay_log_info *const rli,
...
#if !defined(MYSQL_CLIENT) && defined(HAVE_REPLICATION)
int Write_rows_log_event_old::do_before_row_operations(const Slave_reporting_capability *const)
{
    int error = 0;
    /*
      We are using REPLACE semantics and not INSERT IGNORE semantics
...
int Write_rows_log_event_old::do_after_row_operations(const Slave_reporting_capability *const,
        int error)
{
    int local_error = 0;
...
int Write_rows_log_event_old::do_exec_row(const Relay_log_info *const rli)
{
...
int Delete_rows_log_event_old::do_before_row_operations(const Slave_reporting_capability *const)
{
...
int Delete_rows_log_event_old::do_after_row_operations(const Slave_reporting_capability *const,
        int error)
{
    /*error= ToDo:find out what this should really be, this triggers close_scan in nbd, returning error?*/
...
int Update_rows_log_event_old::do_before_row_operations(const Slave_reporting_capability *const)
{
    if (m_table->s->keys > 0) {
        // Allocate buffer for key searches
        m_key = (uchar *)my_malloc(m_table->key_info->key_length, MYF(MY_WME));
...
int Update_rows_log_event_old::do_after_row_operations(const Slave_reporting_capability *const,
        int error)
{
    /*error= ToDo:find out what this should really be, this triggers close_scan in nbd, returning error?*/
    m_table->file->ha_index_or_rnd_end();
...
int Update_rows_log_event_old::do_exec_row(const Relay_log_info *const rli)
{
    DBUG_ASSERT(m_table != NULL);
