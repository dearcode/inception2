...
static int setval(const struct my_option *, void *, char *, my_bool, char *errmsg);
...
{
    return my_handle_options(argc, argv, longopts, get_one_option, NULL, NULL);
}
    ...
    */
    ...
    for (pos = *argv, pos_end = pos + *argc; pos != pos_end ; pos++) {
        ...
        if (cur_arg[0] == '-' && cur_arg[1] && !end_of_options) { /* must be opt */
            ...
            if (*cur_arg == '-') {    /* check for long option, */
                ...
                if (!(opt_found = findopt(opt_str, length, &optp, &prev_found))) {
                            ...
                            if (!getopt_compare_strings(special_opt_prefix[i], opt_str,
                                ...
                                if ((opt_found = findopt(opt_str, length, &optp, &prev_found))) {
                                    if (opt_found > 1) {
                                        ...
                                        if (errmsg)
                                            sprintf(errmsg, "%s: ambiguous option '--%s-%s' (--%s-%s)",
                                                    my_progname, special_opt_prefix[i],
                                                    opt_str, special_opt_prefix[i],
                                                    prev_found);
                    ...
                    if (!opt_found) {
                        ...
                        if (must_be_var) {
                            ...
                            if (errmsg)
                                sprintf(errmsg, "%s: unknown variable '%s'", my_progname, cur_arg);
                            ...
                            if (errmsg)
                                sprintf(errmsg, "%s: unknown option '--%s'", my_progname, cur_arg);
                ...
                if (opt_found > 1) {
                    if (must_be_var) {
                        ...
                        if (errmsg)
                            sprintf(errmsg, "%s: variable prefix '%s' is not unique", my_progname, opt_str);
                        ...
                        if (errmsg)
                            sprintf(errmsg, "%s: ambiguous option '--%s' (%s, %s)",
                ...
                if ((optp->var_type & GET_TYPE_MASK) == GET_DISABLED) {
                    ...
                    if (errmsg)
                        sprintf(errmsg, "%s: %s: Option '%s' used, but is disabled\n", my_progname,
                                option_is_loose ? "WARNING" : "ERROR", opt_str);
                ...
                if (optp->arg_type == NO_ARG) {
                    ...
                    if (optend && (optp->var_type & GET_TYPE_MASK) != GET_BOOL) {
                        ...
                        if (errmsg)
                            sprintf(errmsg, "%s: option '--%s' cannot take an argument",
                                    my_progname, optp->name);
                ...
                for (optend = cur_arg; *optend; optend++) {
                    ...
                    for (optp = longopts; optp->name; optp++) {
                        if (optp->id && optp->id == (int) (uchar) *optend) {
                            ...
                            if ((optp->var_type & GET_TYPE_MASK) == GET_DISABLED) {
                                ...
                                if (errmsg)
                                    sprintf(errmsg, "%s: ERROR: Option '-%c' used, but is disabled\n",
                                            my_progname, optp->id);
                    ...
                    if (!opt_found) {
                        if (my_getopt_skip_unknown) {
                            ...
                            if (errmsg)
                                sprintf(errmsg, "%s: unknown option '-%c'", my_progname, *optend);
            ...
            if ((error = setval(optp, value, argument, set_maximum_value, errmsg)))
                return error;
...
static int setval(const struct my_option *opts, void *value, char *argument,
                  my_bool set_maximum_value, char *errmsg)
{
    int err = 0, res = 0;
    bool error = 0;
    ...
    if (value) {
        if (set_maximum_value && !(value = opts->u_max_value)) {
            ...
            if (errmsg)
                sprintf(errmsg, "%s: Maximum value of '%s' cannot be set", my_progname, opts->name);
        ...
        case GET_BOOL: /* If argument differs from 0, enable option, else disable */
            *((my_bool *) value) = get_bool_argument(opts, argument, &error);
            ...
            if (errmsg)
                sprintf(errmsg, "option '%s': boolean value '%s' wasn't recognized. Set to OFF.",
                        opts->name, argument);
...
ret:
    ...
    if (errmsg)
        sprintf(errmsg, "%s: Error while setting value '%s' to '%s'",
                my_progname, argument, opts->name);
