                                  ...
                                  ulong setup_tables_done_option)
{
    ...
    /* Calls JOIN::prepare */
    DBUG_RETURN(0/*lex->unit.prepare(thd, 0, setup_tables_done_option)*/);
}
...
static bool select_like_stmt_test_with_open(Prepared_statement *stmt,
        TABLE_LIST *tables,
        int (*specific_prepare)(THD *thd),
        ulong setup_tables_done_option)
{
...
bool Select_fetch_protocol_binary::send_data(List<Item> &fields)
{
    Protocol *save_protocol = thd->protocol;
    bool rc;
    thd->protocol = &protocol;
...
bool Prepared_statement::set_db(const char *db_arg, uint db_length_arg)
{
    /* Remember the current database. */
...
bool Protocol_local::send_ok(uint server_status, uint statement_warn_count,
                             ulonglong affected_rows, ulonglong last_insert_id,
                             const char *message)
{
    /*
...
bool Protocol_local::send_error(uint sql_errno, const char *err_msg, const char *)
{
    /*
