...
static
bool set_user_salt(ACL_USER *acl_user, const char *password, uint password_len)
{
    bool result = false;
...
void optimize_plugin_compare_by_pointer(LEX_STRING *plugin_name)
{
    ...
    if (my_strcasecmp(system_charset_info, sha256_password_plugin_name.str,
        ...
        } else if (my_strcasecmp(system_charset_info, old_password_plugin_name.str,
                                 plugin_name->str) == 0) {
...
static void acl_update_proxy_user(ACL_PROXY_USER *new_value, bool is_revoke)
{
    mysql_mutex_assert_owner(&acl_cache->lock);
...
static void acl_insert_proxy_user(ACL_PROXY_USER *new_value)
{
...
inline GRANT_NAME *routine_hash_search(const char *host, const char *ip, const char *db,
...
inline GRANT_TABLE *table_hash_search(const char *host, const char *ip, const char *db,
...
struct MPVIO_EXT : public MYSQL_PLUGIN_VIO
{
    ...
    enum
    { SUCCESS, FAILURE, RESTART } status;
