...
st_select_lex *Item_singlerow_subselect::invalidate_and_restore_select_lex()
{
...
Item_subselect::trans_res Item_in_subselect::single_value_transformer(JOIN *join,
        Comp_creator *func)
{
    SELECT_LEX *select_lex = join->select_lex;
...
Item_subselect::trans_res Item_in_subselect::single_value_in_to_exists_transformer(JOIN *join, Comp_creator *func)
{
    SELECT_LEX *select_lex = join->select_lex;
    THD *const thd = unit->thd;
...
{
    bool result = 0;
    //water is too deep, return in advance
    return 0;
...
bool subselect_union_engine::prepare()
{
    ...
    return 0/*unit->prepare(thd, result, SELECT_NO_UNLOCK)*/;
}
...
bool subselect_union_engine::exec()
{
    ...
    DBUG_ASSERT(thd == item->unit->thd);
    char const *save_where = thd->where;
    const bool res = 0/*(unit->optimize() || unit->exec())*/;
    thd->where = save_where;
    return res;
}
...
{
    item = si;
    int rc = 0/*unit->change_result(res, result)*/;
    result = res;
    return rc;
}
